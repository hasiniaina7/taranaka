...
- [ ] T032 [US3] Implement `LineageShow` in `app/Livewire/Lineages/Show.php` + `resources/views/livewire/lineages/show.blade.php` (replacing the T023 stub): header (name, description, cover), paginated member list (name, lifespan, link to profile) fed by `$lineage->people()->get()->filter(fn ($p) => \App\Support\PersonPrivacy::isPubliclyVisible($p))` — or the temporary `! $person->isDeceased()` check per spec.md's Assumptions if spec 003's `PersonPrivacy` hasn't landed yet — never fed unfiltered (FR-009), skeleton-row loading state, empty state when zero *visible* members ("Aucune personne rattachée pour l'instant") (depends on T023, T009)
- [ ] T032a [P] [US3] Write the feature test in `tests/Feature/Lineage/LineageShowExcludesLivingNonOptedInPeopleTest.php`: a lineage with one deceased and one living non-opted-in person shows only the deceased person on `GET /lineages/{lineage:slug}`; the living person's name/lifespan never appears in the response body (FR-009 privacy filter) (depends on T030)
