...
- [ ] T005 [P] Implement `app/Models/Contribution.php`: `$fillable`, `casts()` method (per Laravel 12 convention — `reviewed_at` as datetime), `author(): BelongsTo` (User), `reviewer(): BelongsTo` (User, nullable), a `target()` accessor resolving `target_type`/`target_id` explicitly (not `morphTo()`, per research.md), and `isStillApplicable(): bool` computed at render time (data-model.md).
