...
1. In an incognito/signed-out browser session, visit `/p/{id}` for a
...
1. As a signed-out visitor, visit `/p/{id}` for a person with no
   `dod`/`yod`.
2. **Expected**: address/phone/exact dob absent from the raw HTML
   (`curl -s .../p/{id} | grep -c "<street value>"` returns 0); a
