...
- [ ] T004 Write `PersonSimilarityScorerTest` unit tests in `tests/Unit/PersonSimilarityScorerTest.php` covering (formula `score = nameScore*0.3 + birthYearScore*0.7`, `birthYearScore = max(0, 1 - gapInYears/40)`, per research.md's corrected weighting): same name + matching birth year scores exactly `1.0` (>= 0.75 acknowledgment-gate boundary); same name + 20-year birth-year gap scores `0.65` (below the 0.75 gate, in the 0.4–0.75 low-confidence band — satisfies SC-002 "no warning"); same name + 40-year gap scores `0.3` (below the 0.4 low-confidence floor — not shown, satisfies User Story 3 AS1); either birth year unknown falls back to `birthYearScore = 0.5` (score `0.65`, same low-confidence band as the 20-year case). Run `vendor/bin/sail artisan test --compact --filter=PersonSimilarityScorerTest` and confirm all assertions FAIL (class does not exist yet).
